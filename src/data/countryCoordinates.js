// Approximate country centroids (not surveyed — good enough for placing a
// pin roughly in the right country on the globe, not for precise geography).
// "Prefer not to say" and anything unrecognised intentionally has no entry —
// GlobeModal skips pins it can't place.
export const COUNTRY_COORDINATES = {
  ...
  "Equatorial Guinea": { lat: 1.6, lng: 10.3 },
...
// Deterministic pseudo-random offset so multiple check-ins from the same
// country don't stack exactly on top of each other on the globe — seeded by
// the check-in id so it's stable across renders/reloads.
function seededOffset(seedStr) {
  ...
  return { dLat: (rand() - 0.5) * 4, dLng: (rand() - 0.5) * 4 };
}

export function getCheckinCoordinates(country, seed) {
